    ...
    if (drag_source_canvas && target && target != drag_source_canvas &&
        ...
        if (moved == 0) {
            ...
            if (stat(info_path, &st_info) == 0) {
                ...
                const char *fallback_def = definfo_for_file(name_only, false);
...
static int copy_file(const char *src, const char *dst) {
    ...
    int rc = -1;
    struct stat st;
    if (stat(src, &st) != 0 || !S_ISREG(st.st_mode)) goto out;
    in_fd = open(src, O_RDONLY);
    if (in_fd < 0) goto out;
    // Create with 0600 and then fchmod to preserve original mode
    out_fd = open(dst, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    if (out_fd < 0) goto out;
    char buf[1 << 16];
    ssize_t r;
...
static int move_file_to_directory(const char *src_path, const char *dst_dir, char *dst_path, size_t dst_sz) {
    ...
    if (rename(src_path, dst_path) != 0) {
        if (errno == EXDEV) {
            // Cross-filesystem: force overwrite by copying then removing source
            if (copy_file(src_path, dst_path) != 0) { perror("[amiwb] copy (move) failed"); unlink(dst_path); return -1; }
            if (unlink(src_path) != 0) { perror("[amiwb] unlink source after copy failed"); /* keep file but continue */ }
        } else {
