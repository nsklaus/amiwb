...
// Forward declaration for checkerboard pattern initialization
static void create_checkerboard_pattern(RenderContext *ctx);
    ...
    if (!is_client_frame) {*/
        ...
        if ((canvas->type == DESKTOP || canvas->type == WINDOW) && !canvas->scanning){
            ...
            if (canvas->type == WINDOW && canvas->view_mode == VIEW_NAMES) {
                ...
                if (font && canvas->xft_draw) {
                    // Use cached XftDraw instead of creating new one
                    int row_h = font->ascent + font->descent + 6;
                        for (int i = 0; i < icon_count; i++) {
                            FileIcon *icon = icon_array[i];
                            if (!icon || icon->display_window != canvas->win) continue;
                            // int rx = icon->x;  // Used in render_x calculation, currently unused
                            int ry = icon->y;
                            // int render_x = BORDER_WIDTH_LEFT + rx - canvas->scroll_x;  // Currently recalculated as sel_x/text_x
                            ...
                            XftDrawStringUtf8(canvas->xft_draw, &xftfg, font, text_x, baseline,
