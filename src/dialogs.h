...
typedef struct RenameDialog {
    ...
    char text_buffer[NAME_MAX + 1];    // Filename buffer (or message for delete)
    char original_name[NAME_MAX + 1];  // Original filename (for display only)
    ...
    char completion_prefix[NAME_MAX];  // The prefix being completed
