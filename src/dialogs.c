...
void show_rename_dialog(const char *old_name, 
    ...
    strncpy(dialog->original_name, old_name, NAME_MAX);
    dialog->original_name[NAME_MAX] = '\0';
...
static void find_completions(RenameDialog *dialog, const char *partial) {
    ...
    strcpy(dialog->completion_prefix, partial);
...
static void apply_completion(RenameDialog *dialog, int index) {
    ...
    strcat(completed, dialog->completion_candidates[index]);
    
    // Replace text buffer
    strcpy(dialog->text_buffer, completed);
    dialog->cursor_pos = strlen(completed);
...
static void render_text_content(RenameDialog *dialog, Picture dest, 
    ...
    if (dialog->dialog_type == DIALOG_DELETE_CONFIRM) {
        ...
        XftDrawStringUtf8(draw, &xft_text, font, warning_x, line_y, 
        ...
        XftDrawStringUtf8(draw, &xft_text, font, text_left_x, line_y,
...
void render_completion_dropdown(Canvas *canvas) {
    ...
    for (int i = 0; i < max_items && start_item + i < dialog->completion_count; i++) {
        ...
        XftDrawStringUtf8(draw, &xft_text, font, text_x, text_y,
...
void show_delete_confirmation(const char *message,
    ...
    strncpy(dialog->text_buffer, message, NAME_MAX);
    dialog->text_buffer[NAME_MAX] = '\0';
    dialog->cursor_pos = 0;
    dialog->selection_start = -1;
    dialog->selection_end = -1;
